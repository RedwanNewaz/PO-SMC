...
class RobotModel(Model):
    def __init__(self, args):
        ...
        self.map_text, dimensions = config_parser_robot.parse_map(self.robot_config["ROBOT"]['map_file'])
        self.half_efficiency_distance = self.robot_config["ROBOT"]['half_efficiency_distance']
        self.thres = self.robot_config["ROBOT"]['threshold']
        self.cell_reward = self.robot_config["REWARD"]
        self.num_actions = self.robot_config["ROBOT"]["num_actions"]
        print(self.cell_reward)
    ...
    def initialize(self):
        ...
        #STEP last row is goal
        row = self.n_rows
        # self.num_states = pow(2,
        #                       self.num_obs_cells) * row * self.n_cols
        self.num_states = ncr(row * self.n_cols,
                              self.num_obs_cells) * row * self.n_cols
