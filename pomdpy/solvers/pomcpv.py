...
class POMCPV(BeliefTreeSolver):
    """
    Monte-Carlo Tree Search implementation, from POMCPV
    """
    ...
    def __init__(self, agent):
        """
        Initialize an instance of the POMCPV solver
        :param agent:
        :param model:
        :return:
        """
        super(POMCPV, self).__init__(agent)

        # Pre-calculate UCB values for a speed-up
        self.fast_UCB = [[None for _ in range(POMCPV.UCB_n)] for _ in range(POMCPV.UCB_N)]

        for N in range(POMCPV.UCB_N):
            for n in range(POMCPV.UCB_n):
                if n is 0:
    ...
    def find_fast_ucb(self, total_visit_count, action_map_entry_visit_count, log_n):
        ...
        if total_visit_count < POMCPV.UCB_N and action_map_entry_visit_count < POMCPV.UCB_n:
