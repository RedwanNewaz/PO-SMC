...
class DiscreteActionMapping(ActionMapping):
    ...
    def __init__(self, belief_node_owner, discrete_action_pool, bin_sequence):
        ...
        for bin_number in self.bin_sequence:
            self.entries.get(bin_number.bin_number).is_legal = True
...
class DiscreteActionMappingEntry(ActionMappingEntry):
    ...
    def __init__(self):
        ...
        # TODO add p_value step for SMC
        self.total_p_value =0
        self.mean_p_value = 0
        self.visit_p_count = 0
...
#TODO STEP for SMC
    def update_p_value(self, delta_total_p):
        if delta_total_p == 0:
            return False
        assert np.isfinite(delta_total_p)

        # Add up the P value
        self.total_p_value += delta_total_p
        self.visit_p_count += 1

        # Update the mean Q
        old_mean_p = self.mean_p_value
        # if self.visit_count <= 0:
        #    self.mean_q_value = -np.inf
        # else:

        # Average the P value by taking the Total P value of this entry divided by the
        # number of times this action has been tried
        if(self.visit_count> minimum_particles):
            self.mean_p_value = old_div(self.total_p_value, self.visit_p_count)
        else:
            self.mean_p_value = old_div(self.total_p_value, 250.0)

        return self.mean_p_value != old_mean_p
